...
public class Datacenter extends SimEntity {
	...
	protected void processDataDelete(SimEvent ev, boolean ack) {
		...
		} else { // if an error occured, notify broker
	...
	/**
	 * Process the event for an broker/Broker who wants to know the status of a Cloudlet. This
	 * PowerDatacenter will then send the status back to the broker/Broker.
	 * 
	 * @param ev a Sim_event object
	 * @pre ev != null
	 * @post $none
	...
	protected void processCloudletStatus(SimEvent ev) {
		...
		try {
			...
			status = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId).getCloudletScheduler()
		...
		// if a sender using normal send() methods
			...
			try {
				...
				brokerId = cl.getbrokerId();

				status = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId)
	...
	/**
	 * Process the event for an broker/Broker who wants to create a VM in this PowerDatacenter. This
	 * PowerDatacenter will then send the status back to the broker/Broker.
	 * 
	 * @param ev a Sim_event object
	 * @param ack the ack
	...
	/**
	 * Process the event for an broker/Broker who wants to destroy a VM previously created in this
	...
	/**
	 * Process the event for an broker/Broker who wants to migrate a VM. This PowerDatacenter will
	 * then send the status back to the broker/Broker.
	 * 
	 * @param ev a Sim_event object
	 * @pre ev != null
	 * @post $none
	...
	protected void processCloudlet(SimEvent ev, int type) {
		...
		switch (type) {
			case CloudSimTags.CLOUDLET_CANCEL:
				processCloudletCancel(cloudletId, brokerId, vmId);
				break;

			case CloudSimTags.CLOUDLET_PAUSE:
				processCloudletPause(cloudletId, brokerId, vmId, false);
				break;

			case CloudSimTags.CLOUDLET_PAUSE_ACK:
				processCloudletPause(cloudletId, brokerId, vmId, true);
				break;

			case CloudSimTags.CLOUDLET_RESUME:
				processCloudletResume(cloudletId, brokerId, vmId, false);
				break;

			case CloudSimTags.CLOUDLET_RESUME_ACK:
				processCloudletResume(cloudletId, brokerId, vmId, true);
				break;
			default:
				break;
		}
	...
	/**
	 * Process the event for an broker/Broker who wants to move a Cloudlet.
	 * 
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		int brokerId = array[1];
		int vmId = array[2];
		int vmDestId = array[3];
		int destId = array[4];

		// get the cloudlet
		Cloudlet cl = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId)
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (cl.getCloudletStatus() == Cloudlet.SUCCESS) {// if yes, send it back to broker
				int[] data = new int[3];
				data[0] = getId();
				data[1] = cloudletId;
				data[2] = 0;
			...
			if (destId == getId()) {
				Vm vm = getVmAllocationPolicy().getHost(vmDestId, brokerId).getVm(vmDestId,brokerId);
				if (vm == null) {
					failed = true;
				} else {
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			int brokerId = cl.getbrokerId();
			int vmId = cl.getVmId();

			// time to transfer the files
			double fileTransferTime = predictFileTransferTime(cl.getRequiredFiles());

			Host host = getVmAllocationPolicy().getHost(vmId, brokerId);
			Vm vm = host.getVm(vmId, brokerId);
	 ...
	 * @param brokerId ID of the cloudlet's owner
	 * @param ack $true if an ack is requested after operation
	 * @param vmId the vm id
	 * @pre $none
	 * @post $none
	 */
	protected void processCloudletResume(int cloudletId, int brokerId, int vmId, boolean ack) {
		double eventTime = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId)
		...
		if (ack) {
			...
			sendNow(brokerId, CloudSimTags.CLOUDLET_RESUME_ACK, data);
		}
	}
	 ...
	 * @post $none
	 */
	protected void processCloudletPause(int cloudletId, int brokerId, int vmId, boolean ack) {
		boolean status = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId)
		...
		if (ack) {
			...
			sendNow(brokerId, CloudSimTags.CLOUDLET_PAUSE_ACK, data);
		}
	}
	 ...
	 * @post $none
	 */
	protected void processCloudletCancel(int cloudletId, int brokerId, int vmId) {
		Cloudlet cl = getVmAllocationPolicy().getHost(vmId, brokerId).getVm(vmId,brokerId)
				.getCloudletScheduler().cloudletCancel(cloudletId);
		sendNow(brokerId, CloudSimTags.CLOUDLET_CANCEL, cl);
	}
