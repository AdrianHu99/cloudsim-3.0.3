...
public class Host {
	...
	public void updateTotalProfit(double price){
		totalProfit += price;
	}
	
	protected HashMap<Integer, ArrayList<Double>> msop = new HashMap<Integer, ArrayList<Double>>();
	
	
	public HashMap<Integer, ArrayList<Double>> getmsop(){
		return msop;
	}
	
	public void addmsop(int userid, double record){
		//int b = getmsop().get(userid).size();
		getmsop().get(userid).add(record);
		getmsop().put(userid, getmsop().get(userid));
	}
	
	
	
	protected HashMap<Integer, ArrayList<Double>> maop = new HashMap<Integer, ArrayList<Double>>();
	
	
	public HashMap<Integer, ArrayList<Double>> getmaop(){
		return maop;
	}
	
	public void addmaop(int userid, double record){
		//int b = getmaop().get(userid).size();
		getmaop().get(userid).add(record);
		getmaop().put(userid, getmaop().get(userid));
	}
	
	
	
	
	protected HashMap<Integer, Integer> mtimes = new HashMap<Integer, Integer>();
	
	public void addmtimes(int userid){
		int times = getmtimes().get(userid);
		times++;
		mtimes.put(userid, times);
	}
	
	public void minusmtimes(int userid){
		int times = getmtimes().get(userid);
		times--;
		mtimes.put(userid, times);
	}
	
	public HashMap<Integer, Integer> getmtimes(){
		return mtimes;
	}
	...
	public Host(
		...
		setFailed(false);
		totalProfit = 0;
		msop = new HashMap<Integer, ArrayList<Double>>();
		maop = new HashMap<Integer, ArrayList<Double>>();
		mtimes = new HashMap<Integer, Integer>();
	}
